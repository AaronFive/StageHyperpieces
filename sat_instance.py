...
def make_corresp_dictionnaries(n, m, pi_size, pi):
...
def make_sat_instance(comments, string_1, string_2):
    ...
    pi = get_pi(pi_size)
    x_dict, y_dict = make_corresp_dictionnaries(n, m, pi_size, pi)
    comment_string = "\n c ".join(comments)
    ...
    for i in range(n):
        for j1 in range(m):
            for j2 in range(m):
                if j1 != j2:
                    clauses_i.append(no_double_i_clause(x_dict, i, j1, j2))
    clauses_i_string = f"\n {top} ".join(clauses_i)
    ...
    for j in range(m):
        for i1 in range(n):
            for i2 in range(n):
                if i1 != i2:
                    clauses_j.append(no_double_j_clause(x_dict, i1, i2, j))
    clauses_j_string = f" \n {top} ".join(clauses_j)
        ...
        for i2 in range(i1 + 1, n):
            for j1 in range(m):
                for j2 in range(j1 + 1, m):
                    clauses_crossings.append(no_crossing_clause(x_dict, i1, i2, j1, j2))
    clauses_crossings_string = f"\n {top} ".join(clauses_crossings)
    ...
    for a in pi:
        for b in pi:
            for c in pi:
                if b != c:
                    clauses_function.append(function_clause(y_dict, a, b, c))
    clauses_function_string = f"\n {top} ".join(clauses_function)

    # Match
    clauses_match = []
    for i in range(n):
        for j in range(m):
            clauses_match.append(match_clause(x_dict, y_dict, i, j, string_1, string_2))
    clauses_match_string = f"\n {top} ".join(clauses_match)
    ...
    header = f"wcnf {nbvar} {nb_clauses} {top}"
    ...
    for i in range(n):
        for j in range(m):
            clauses_max.append(str(x_dict[i, j]))
    clauses_max_string = f"\n 1".join(clauses_max)
